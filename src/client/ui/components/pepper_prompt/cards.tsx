...
interface CardProps {
  ...
  pressedCallback?: (option: CardOption) => void
}
  ...
  return (
    <imagebutton
      ...
      Event={{
        ...
        MouseLeave: () => { setHover(false); setPress(false) },
        MouseButton1Down: () => { setPress(true); pressedCallback(info) },
...
export function Cards({
  ...
  const [second, secondMotion] = useMotion(first.getValue());
  const [third, thirdMotion] = useMotion(second.getValue())
